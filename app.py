...
st.markdown("""
    ...
    /* Styles for the HTML output generated by Gemini */
    .ai-synthesis-box ul { margin: 0; padding: 0; list-style-type: none; }
    .ai-synthesis-box li { margin-bottom: 0.8rem; font-size: 1rem; color: #3c4043; line-height: 1.6; }
    .ai-synthesis-box strong { color: #202124; font-weight: 700; }
...
def generate_ai_brief(metrics_dict):
    try:
        ...
        if not api_key:
            return "<ul><li>⚠️ <strong>API Key Missing:</strong> Configure GEMINI_API_KEY in Streamlit Secrets.</li></ul>"
        
        client = genai.Client(api_key=api_key)
        
        # We pre-calculate rates so the AI has concrete analytical leverage
        ...
        Rule 1: You MUST cite the exact numbers provided above (e.g., 'The {stop_rate}% stop rate...', 'Out of {metrics_dict['street']} impressions...').
        Rule 2: Identify exactly where the customer is dropping off in the funnel.
        Rule 3: Output strictly in this raw HTML format (no markdown, no ```html blocks, just the raw HTML list):
        
        <ul>
            <li>📊 <strong>Funnel Diagnostics:</strong> [Your number-driven analysis of the traffic flow]</li>
            <li>⚠️ <strong>Revenue Leakage:</strong> [Identify the exact bottleneck using data]</li>
            <li>💡 <strong>Actionable Fix:</strong> [Provide a concrete retail strategy based on the bottleneck and Dominant OS]</li>
        </ul>
        """
        
        response = client.models.generate_content(model='gemini-2.5-flash', contents=prompt)
        
        # Sanitize output to guarantee it renders properly in UI
        html_output = response.text.replace("```html", "").replace("```", "").strip()
        return html_output
    except Exception as e:
        return "<ul><li>⚠️ <strong>AI Unavailable:</strong> Rate limit reached or connection issue.</li></ul>"
...
if df.empty:
    ...
    if ai_enabled:
        ...
        with st.spinner("Generating AI Analysis..."):
            ai_html_content = generate_ai_brief(metrics_payload)
            
        # UI FIX: Injecting valid HTML prevents markdown rendering errors
        st.markdown(f"""
            <div class="ai-synthesis-box">
                <h4>✨ AI Executive Brief</h4>
                {ai_html_content}
            </div>
        """, unsafe_allow_html=True)
