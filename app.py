...
You will receive a structured intelligence report generated by an analytics engine.

Your job is ONLY to convert that structured intelligence into an executive narrative.

Rules:
- Do not invent numbers
- Use the interpretation provided
...
**Traffic interpretation:** explain traffic quality

**Visit behavior:** explain visit quality and dwell

**Commercial outcome:** explain conversion outcome

**Primary bottleneck:** choose one

**Recommended action:** one practical action
"""
...
def compute_retail_intelligence(metrics: dict) -> dict:
    walk_by = metrics["walk_by"]
    interest = metrics["interest"]
    near_store = metrics["near_store"]
    visits = metrics["visits"]
    qualified = metrics["qualified_visits"]
    engaged = metrics["engaged_visits"]
    dwell = metrics["avg_dwell_seconds"]
    ...
    qualified_rate = qualified / visits if visits else 0
    engaged_rate = engaged / visits if visits else 0
    conversion = transactions / visits if visits else 0

    if tii >= 70:
        traffic_health = "strong"
    elif tii >= 45:
        traffic_health = "moderate"
    else:
        traffic_health = "weak"

    if engaged_rate >= 0.35 or vqi >= 70:
        visit_quality = "strong"
    elif engaged_rate >= 0.18 or vqi >= 45:
        visit_quality = "moderate"
    else:
        visit_quality = "weak"
    ...
    if conversion >= 0.20:
        ...
        commercial_outcome = "moderate"
    else:
        commercial_outcome = "weak"

    if traffic_health == "weak":
        primary_issue = "low traffic capture"
        action = "Improve storefront visibility, frontage communication, and awareness-driving triggers."
    elif visit_quality == "weak":
        primary_issue = "weak in-store engagement"
        action = "Improve first-touch merchandising, assisted selling, and product discovery inside the store."
    elif commercial_outcome == "weak":
        primary_issue = "weak sales conversion"
        action = "Strengthen closing tactics, offer communication, and staff-led conversion moments."
    else:
        primary_issue = "no major structural weakness"
        action = "Maintain current operating discipline and scale traffic while preserving visit quality."
    ...
    return {
        ...
        "wb": round(walk_by, 2),
        "si": round(interest, 2),
        "ns": round(near_store, 2),
        "v": int(round(visits)),
        "qv": int(round(qualified)),
        "ev": int(round(engaged)),
        "qr": round(qualified_rate * 100, 1),
        "er": round(engaged_rate * 100, 1),
        "cv": round(conversion * 100, 1),
        "dwell_sec": round(float(dwell), 1),
        "txn": int(round(transactions)),
        "rev": round(float(revenue), 0),
        "tii": round(float(tii), 1),
        "vqi": round(float(vqi), 1),
        "sai": round(float(sai), 1),
        "aqi": round(float(aqi), 1),
    }


@st.cache_data(ttl=300)
def generate_ai_brief(structured_intelligence: dict) -> str:
    if not GEMINI_API_KEY or genai is None:
        return "⚠️ **AI unavailable:** GEMINI_API_KEY not configured or Gemini SDK unavailable."

    try:
        client = genai.Client(api_key=GEMINI_API_KEY)
        cache_name = get_or_create_ai_cache()
        payload = json.dumps(structured_intelligence, separators=(",", ":"))
...
raw_ai_metrics = {
    ...
    "revenue": value,
    "tii": traffic_intelligence_index,
    "vqi": visit_quality_index,
    "sai": store_attraction_index,
    "aqi": audience_quality_index,
}
